...
@section('content')
    <div class="row py-5">
        ...
        <div class="col-12 col-md-7 order-1 order-md-0" id="payment_cal_path">
                        ...
                        <div id="payment_areas">
                            ...
                            <div class="payment_area nowp_parea">
                                <div class="choice-radio"><label for="choice_nowp"><input checked type="radio"
                                        ...
                                        <span><img style="max-width: 50px" src="/img/icons/awallex.png"> {{ @$pagstn['airwallex_name'] }}
                                        </span></label></div>
                                <div class="payment_body bag892" id="nowp-container">
                                    Click the button below to complete payment. This will redirect you to Airwallex checkout
                                    page.
                            ...
                            @if ($show_bank)
                                <div class="payment_area bank_parea">
                                    ...
                                    <div class="payment_body  d-none" id="bank-container">
                                        This will provide you with account details where the payment will be manually
                                        approved.
                                    </div>
                                </div>
                            @endif
            ...
            <div class="text-end mt-3"><button id="complete_payment" class="btn  btn-primary pay_btn py-3 px-4"> Complete
                    Payment</button></div>
                                    ...
                                    <th class="cfw-cart-item-description">
                                        ...
                                        <div class="cfw-cart-item-data">
                                            <div class="variationx">
                                                ...
                                                <div class="dd">
                                                    {{ date('h:m a', strtotime($quote->start_date . ' ' . $quote->start_time)) }}
                                                </div>
                                                <div class="dt">End Date:</div>
                                                <div class="dd">{{ date('d-m-Y', strtotime($quote->end_date)) }}</div>
                                                <div class="dt">End Time:</div>
                                                <div class="dd">
                                                    {{ date('h:m a', strtotime($quote->end_date . ' ' . $quote->end_time)) }}
                                                </div>
                                                ...
                                                <div class="dd">{{ $quote->title }} {{ $quote->first_name }}
                                                    {{ $quote->middle_name }} {{ $quote->last_name }}</div>
    ...
    <script>
        ...
        (async () => {
            ...
            const {
                ...
                env: '{{ config('app.env') == 'locald' ? 'demo' : 'prod' }}', // Setup which Airwallex env('staging' | 'demo' | 'prod') to integrate with
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        payments.redirectToCheckout({
                            env: '{{ config('app.env') == 'locald' ? 'demo' : 'prod' }}',
                            mode: 'payment',
                            currency: 'GBP', // Typo fixed: was 'curremcy'
                            country_code: 'GB', // Country codes are usually uppercase
                            intent_id: data.intent_id, // Required
                            client_secret: data.client_secret, // Required
                            successUrl: '{{ url('/airwallex/confirmed') }}', // Must be HTTPS
                            failUrl: '{{ url('/airwallex/cancelled') }}', // Must be HTTPS
                            ...
                            applePayRequestOptions: {
                                ...
                                countryCode: 'GB',  // The merchant's two-letter ISO 3166 country code. Like 'HK'
                                totalPriceLabel: 'MONZIC SOLUTIONS LTD', // Provide a business name for the label field.
                            },

                            googlePayRequestOptions: {
                                countryCode: "GB",
                                merchantInfo: {
                                    merchantName: "MONZIC SOLUTIONS LTD",
                                },
                                emailRequired: true,
        ...
        async function userAuthentication() {
            ...
            try {
                const res = await fetch('/airwallex/registration', {
                    method: 'POST',
                    headers: {
