...
if($('#myTable').length){
	...
	table = $('#myTable').DataTable( {
		...
		'initComplete' : function(setting, json){ //When table has been fully initialize
			// set_up_data_plugs();
			$('.dt-buttons').before('<h2>Coupons</h2>');
		},
		//responsive: true,
		...
		columns: [
			...
			{ data: ddt.promo_code, name: 'promo_code'},
			{ data: ddt.amount, name: 'amount', render: function(data,b,c){ return data + '('+ c[ddt.discount_type]+ ')'}},
			{ data: ddt.min_spent, name: 'min_spent'},
			{ data: ddt.matches, name: 'matches'},
			{ data: ddt.available, name: 'available'},
			{ data: ddt.used, name: 'used'},
			{ data: ddt.expires, name: 'expires'},
...
function create_row(){
	...
	$('#modal_entry .modal-header h4').html('Create Coupon');
	$('#modal_entry .modal-footer .btn-main').html('<i class="fa fa-plus"></i> Create Coupon');
	global_ch_dir = "POST";
...
function edit_row(event){
	...
	$("#modal_entry").modal('show');
	$('#modal_entry .modal-header h4').html('Edit Coupon');
	$('#modal_entry .modal-footer .btn-main').html('<i class="fa fa-save"></i> Update Coupon');
